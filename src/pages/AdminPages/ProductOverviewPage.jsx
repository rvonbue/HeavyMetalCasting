...
function ProductDisplayRow({ product, productEditFields }) {
  const {productAttributes} =  useSelector(state =>  state.products);

    return (
        <tr key={product.id + "tr"} className="hover:bg-gray-50 transition-colors">
              {productEditFields.map(({ id, column_name, input_type, lookup_name }, index) => (
                <td
                  key={id}
                  className="p-3 border-b border-hmc-button-text-b text-left align-top"
                >
                  {getCellValue({field: { column_name, input_type, lookup_name },  product, productAttributes }) }
                </td>
              ))}
